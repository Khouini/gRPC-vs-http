...
echo "=== Performance Comparison: gRPC vs HTTP (Hotel Data) ==="
echo ""
...
if curl -s http://localhost:3000/stats > /dev/null 2>&1; then
    echo "✓ Node.js hotel services are running"
    NODE_RUNNING=true
else
    echo "✗ Node.js hotel services not running"
    NODE_RUNNING=false
fi

# Check Go services  
if curl -s http://localhost:8080/stats > /dev/null 2>&1; then
    echo "✓ Go hotel services are running"
    GO_RUNNING=true
else
    echo "✗ Go hotel services not running"
    GO_RUNNING=false
fi
...
if [ "$NODE_RUNNING" = true ]; then
    test_api "http://localhost:3000/stats" "Node.js HTTP (Hotels)" 50
fi

if [ "$GO_RUNNING" = true ]; then
    test_api "http://localhost:8080/stats" "Go gRPC (Original Hotels)" 50
    test_api "http://localhost:8080/stats-streaming?chunkSize=100" "Go gRPC (Streaming 100 chunks)" 50
    test_api "http://localhost:8080/stats-streaming?chunkSize=250" "Go gRPC (Streaming 250 chunks)" 50
fi
...
echo ""
echo "=== Summary ==="
echo "Hotel Data Optimizations applied:"
echo "- Pre-converted protobuf hotel data (eliminates conversion overhead)"
...
echo "- All statistics counting handled in gateway layer"
echo "- Optimized chunk sizes for hotel data (100-250 hotels per chunk)"
